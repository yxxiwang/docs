...
> 我们在这里分别监控了在执行数据平衡这段时候中，所有服务器以及整体集群的 `网络接收` `网络发送` `使用磁盘量` `磁盘度` `磁盘写` 情况。

![](assets/markdown-img-paste-2017102713312025.png)

#### 通过 网络发送 流量监控说明整个测试过程

> 从上文监控数据中可以看出 `网络发送` 和 `网络接收` 是一致的。

![](assets/markdown-img-paste-20171027135452296.png)

#### 磁盘使用监控

> 这里可以看到 `HDFS集群` 和 `NameNode` 磁盘使用量未发生变化；`DN01/DN02/DN03` 磁盘使用量均减少，同时 `DN10`（新加）磁盘使用量上升。

![](assets/markdown-img-paste-20171027140919456.png)
